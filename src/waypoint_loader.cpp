...
public:
	...
	void stateCallback(const waypoint_maker::State::ConstPtr &msg){
		state_inspection = msg->current_state;
	}
	...
	void getNewWaypoints() {
		...
		is_.open("/home/so-yeon/data.csv");
		...
		while(!is_.eof()) {
			...
			if(str_buf != "") {
				...
				new_waypoints_.push_back(temp_waypoint);
			}
		}
		is_.close();
		
		size_ = new_waypoints_.size();
	...
	void getClosestWaypoint(geometry_msgs::PoseStamped current_pose) {
		...
		for(int i=0;i<size_;i++) {
			...
			if(dist < max_search_dist_ && abs((new_waypoints_[i].mission_state - state_inspection)) <= 1 ) closest_waypoint_candidates.push_back(i);
		}
